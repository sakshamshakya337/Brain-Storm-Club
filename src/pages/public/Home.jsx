...
// Derive supporting slots from members list for HomeMembers
// Leadership roles to feature on homepage (ordered by display priority)
// Keywords are matched case-insensitively against the member's role field from the DB
const LEADERSHIP_ROLES = [
  { keywords: ['president'],                                          label: 'President'        },
  { keywords: ['vice president', 'vp'],                              label: 'Vice President'   },
  { keywords: ['secretary'],                                         label: 'Secretary'        },
  { keywords: ['technical head', 'tech head'],                       label: 'Technical Head'   },
  { keywords: ['social media head', 'media head'],                   label: 'Media Head'       },
  { keywords: ['head coordinator', 'head co-ordinator', 'head cord'], label: 'Head Coordinator'},
];

function deriveLeadership(members) {
  if (!members || members.length === 0) return { president: null, supportingSlots: [] };
  const used = new Set();
  ...
  const findByRole = ({ keywords }) =>
    members.find(m => {
      ...
      return keywords.some(k => role === k || role.includes(k));
    });

  let president = null;
  const supportingSlots = [];

  for (const roleDef of LEADERSHIP_ROLES) {
    const member = findByRole(roleDef);
    if (!member) continue;
    used.add(member._id);
    if (!president) {
      president = member;
    } else {
      supportingSlots.push({ member, roleLabel: member.role || roleDef.label });
    }
  }

  // Fallback: if president not found via role, use first member
  if (!president && members.length > 0) {
    president = members[0];
    used.add(president._id);
  }

  return { president, supportingSlots };
}
...
export default function Home() {
  ...
  return <main ref={mainRef} onClick={handleTextClick} className="homepage min-h-screen overflow-x-hidden bg-paper font-body text-ink selection:bg-spark-soft" style={{ perspective: '1200px' }}>
    ...
    {/* Live Events Section */}
    <HomeEvents featuredEvents={featuredEvents} isEventsLoading={isEventsLoading} />
    ...
    <HomeMembers isMembersLoading={isMembersLoading} liveMembers={liveMembers} president={president} supportingSlots={supportingSlots} />
